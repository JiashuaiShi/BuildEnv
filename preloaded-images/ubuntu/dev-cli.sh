...
# 服务容器名称
CONTAINER_NAME="${UBUNTU_CONTAINER_NAME}" # 注意：这里是 UBUNTU_CONTAINER_NAME
# 服务镜像名称
IMAGE_NAME="${UBUNTU_IMAGE_NAME}" # 注意：这里是 UBUNTU_IMAGE_NAME
# 服务 SSH 端口
...
start_container() {
    ...
    if docker-compose up -d; then
        ...
        echo "   端口: ${SSH_PORT}"
        echo "   用户: ${SSH_USER}"
        echo "   密码: (请查看 .env 文件中的 ${UBUNTU_USER_PASSWORD:-UBUNTU_USER_PASSWORD} 配置)"
        echo ""
