...
# AlmaLinux 9 开发环境 (alma9-dev) 管理工具
# 用法: ./dev-cli.sh [命令]
...
# 显示帮助信息
show_help() {
    echo "AlmaLinux 9 开发环境 (alma9-dev) 管理工具"
...
# 构建容器 (如果适用)
build_container() {
    # 旧脚本中这里是 ./1-build.sh，现在统一使用 $BUILD_SCRIPT_PATH
...
start_container() {
    ...
    if docker-compose up -d; then
        ...
        echo "   端口: ${SSH_PORT}"
        echo "   用户: ${SSH_USER}"
        echo "   密码: (请查看 .env 文件中的 ${ALMA9_USER_PASSWORD:-ALMA9_USER_PASSWORD} 配置)"
        echo ""
        echo " 连接命令: ssh -p ${SSH_PORT} ${SSH_USER}@localhost"
        echo " 或使用: ./dev-cli.sh ssh"
        echo "---------------------------------"
        echo ""
        echo "AlmaLinux 9 开发环境特性 (基于 Supervisord):"
        echo "  1. C++ 开发: GCC, Clang, CMake, Ninja, GDB, Valgrind 等"
        echo "  2. Java 开发: JDK 8/11/17 支持"
        echo "  3. Python 开发: Python 3 + pip (通过 Miniconda)"
        echo ""
    else
...
# 清理环境
clean_container() {
    # 旧脚本是 docker-compose down --volumes --remove-orphans --rmi all 2>/dev/null
    # --rmi all 比较危险，暂时只清理容器、网络和卷
    read -p "警告: 此操作将移除属于此服务定义的容器、网络和卷。是否继续? (y/N): " confirm
    if [[ "$confirm" =~ ^[yY](es)?$ ]]; then
        echo "信息: 正在清理环境 (${CONTAINER_NAME})..."
        docker-compose down --volumes --remove-orphans
        echo "信息: 清理完成。"
        echo "提示: 如需移除相关 Docker 镜像, 请手动执行 'docker rmi ${IMAGE_NAME}' 或检查 .env 中的镜像名。"
    else
        echo "信息: 清理操作已取消。"
    fi
}
